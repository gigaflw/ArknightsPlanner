...
_ITEMS = {
    '基础': '1d 至纯源石 理智 1k龙门币 1k经验值',
    '抽卡': '合成玉 寻访凭证',

    '凭证': '资质凭证 高级凭证 采购凭证',
    '作战记录': '基础作战记录 初级作战记录 中级作战记录 高级作战记录',

    'T1': '源岩 破损装置 酯原料 代糖 异铁碎片 双酮',
    'T2': '固源岩 装置 聚酸酯 糖 异铁 酮凝集',
    'T3': '固源岩组 全新装置 聚酸酯组 糖组 异铁组 酮凝集组 RMA70-12 研磨石 轻锰矿 扭转醇',
    'T4': '提纯源岩 改量装置 聚酸酯块 糖聚块 异铁块 酮阵列 RMA70-24 五水研磨石 三水锰矿 白马醇',
    'T5': 'D32钢 双极纳米片 聚合剂',

    '芯片': [f'{cate}{chip}' for cate in JOBS for chip in ('芯片', '双芯片', '芯片组')],
    '突破': '芯片助剂 原装信物 传承信物 遗产信物 皇家信物',
        ...
        with open(CRAWLER_PATH, "r") as f:
            for line in f:
                ...
                if data["stage"]["code"].startswith('GT'):
                    drops['1k龙门币'] = 180/1000
                    drops['骑士金币'] = ap_cost * 10
                else:
                    drops['1k龙门币'] = ap_cost * 10 / 1000
